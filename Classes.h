...
public:
    ...
    void create_subjective()
    {
        ...
        if (!file.is_open())
        {
            cout << "Error opening a file subjective.txt";
            exit(0);
        }
    ...
    void show_test_answers()
    {
       ifstream file("testAnswers.txt");
        ...
        cout << "Choose a test(to see marks): " << endl;
        ...
        while (getline(file, line))
        {
            ...
            if (line.find(query) != string::npos)
            {
                ...
                if (test_name == choosen_test_name)
                {
                    ...
                    {
                        string name = "Name:";
                        name = in_line.substr(name.size());
                        Names[name_found] = name;
                        name_found++;
                        cout << endl;
                        break;
                    }
        ...
        cout << "Choose a name(to see marks): " << endl;
        int choosen_name_num;
        for (int i = 0; i < name_found; i++)
        {
            cout << " " << i << ". " << Names[i] << endl;
        }
        cout << ">";
        cin >> choosen_name_num;
        string choosen_name = Names[choosen_name_num];
        ifstream file1("objective_marks.txt");
        cout << "Name:" << choosen_name << endl;
        while (getline(file1, line))
        {
            string query = "Name:";
            if (line.find(query) != string::npos)
            {
                string student_name = line.substr(query.size());
                if (student_name == choosen_name)
                {
                    string in_line;
                    while (getline(file1, in_line))
                    {
                        if (in_line == "---") {
                            break;
                        }
...
public:
    ...
    void showTests()
    {
        ...
        cout<<"Duration of test is "<<duration<< " seconds"<<endl;
        answersFile << "Name: " << get_name() << endl;
        answersFile << "Test name: " << choosen_test_name << endl;
        answersFile << "Points: " << points << "/" << available_points << endl;
        answersFile << "Duration: "  << duration<<" seconds" << endl
                    << "---" << endl;
    ...
    double duration = 0.0;
        ...
        {
            essay_answers_file << "Name: " << get_name() << endl;
            essay_answers_file << "Answer: " << essay_text << endl;
            essay_answers_file << "Comment: " << comment << endl;
            essay_answers_file << "Duration: " << duration<<" seconds" << endl
                               << "---" << endl;
            cout<<"Duration of test is "<<duration<< " seconds"<<endl;
            cout << "Answer recorded!" << endl;
        }
    ...
    void showSubjective()
    {
        ...
        if (!subjective_file.is_open())
        {
            cout << "Error opening file subjective.txt";
            exit(0);
        }
